...
# proxies={"http": "http://D47456:Sw9s1im9!@pcyvipncp2n.edf.fr:3128/",}
...
class Point(object):
    ...
    @classmethod
    def tryjsonload(self, url, iter=0):
        if iter > 5 :
            raise
        try:
            ...
            print e
            time.sleep(1.0)
            self.tryjsonload(url, iter+1)

    @classmethod
    def search(self, word):
        url = "searchPoints/" + urllib.quote(word.encode('utf8'))

        # req = API._rest_req(url)
        # req = json.loads(req.text)
        req = self.tryjsonload(url)
...
class Itinerary(object):
    ...
    def __new__(cls, start, citystart = None, end = None, cityend = None, date = None, sens = None, via = None, walk_speed = None, with_transport = None, without_transport = None):
        ...
        if citystart != None:
            data['departure']=start
            data['departureCity']=citystart

        if cityend != None:
            data['arrival']=end
            data['arrivalCity']=cityend
...
class API(object):
    ...
    def _rest_req(cls, path, **kwargs):
        ...
        # r = requests.get(urlparse.urljoin("http://www.vianavigo.com/stif_web_carto/rest/", path), proxies=proxies, **kwargs)
        r = requests.get(urlparse.urljoin("http://www.vianavigo.com/stif_web_carto/rest/", path), **kwargs)
